...
class AutonomousCoreTests(unittest.TestCase):
    def test_allowlist_builds_only_make_commands(self):
        self.assertEqual(validation_commands({"unit": "test"}, POLICY), {"unit": ("make", "test")})
    ...
    def test_state_round_trip_resume_and_abort(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            state = RunState(1, "012-test", "feature/test", "a", "b", "digest", "T001", 1,
                             "validate", "unit-test", ("src/a.py",), "failed", directory, "now")
    ...
    def test_failure_classification_and_unsafe_policy(self):
        self.assertEqual(recovery.classify("unit", 1, "AssertionError: failed"), "unit-test")
        self.assertEqual(recovery.classify("command", 127, "command not found"), "dependency")
    ...
    def test_review_schema_validation(self):
        ...
        failed = parse_review(json.dumps({"result": "fail", "findings": [{
            "severity": "high", "category": "spec", "file": "a.py",
            "description": "missing behavior", "required": True}]}))
    ...
    def test_review_repairs_then_passes_and_repeated_findings_stop(self):
        ...
        repairs = []
        self.assertEqual(review_with_repairs(lambda: next(results), repairs.extend, 3).result, "pass")
        self.assertEqual(len(repairs), 1)
        with self.assertRaisesRegex(RuntimeError, "identical"):
            review_with_repairs(lambda: ReviewResult("fail", (finding,)), lambda _: None, 3)
    ...
    def test_review_prompt_embeds_complete_inputs(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (repo / "prompts").mkdir()
            (repo / "schemas").mkdir()
            template = Path(__file__).resolve().parents[1] / "prompts" / "review-feature.md"
            (repo / "prompts" / "review-feature.md").write_text(template.read_text(encoding="utf-8"), encoding="utf-8")
            for name in ("spec.md", "plan.md", "tasks.md", "validation-log.md"):
                content = f"{name}-start\n" + "a" * 13_000 + f"\n{name}-end"
                (feature / name).write_text(content, encoding="utf-8")
            completed = subprocess.CompletedProcess([], 0, '{"result":"pass","findings":[]}', "")
            merge = subprocess.CompletedProcess([], 0, "abc\n", "")
            diff_text = "diff-start\n" + "d" * 21_000 + "\ndiff-end"
            diff = subprocess.CompletedProcess([], 0, diff_text, "")
            with mock.patch("agent.review.subprocess.run", side_effect=(merge, diff, completed)) as run:
    ...
    def test_review_fails_closed_instead_of_truncating_oversized_input(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch("agent.review.subprocess.run", side_effect=(merge, diff)) as run:
                with self.assertRaisesRegex(RuntimeError, "refusing to review truncated content"):
    ...
    def test_stack_detection_and_proposal(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(adapters.detect(repo, [generic, python, node])[0].name, "python")
